...
export const get_akasaka_all = async ({ date_time }:any) => {
  const res = await axios.get(`${apiUrl}/dai_data/store-data/?date_time=${date_time}&store_name=akasaka`);
  return res.data;
};


export const get_boomtengin_all = async ({ date_time }:any) => {
  const res = await axios.get(`${apiUrl}/dai_data/store-data/?date_time=${date_time}&store_name=boomtengin`);
  return res.data;
};
